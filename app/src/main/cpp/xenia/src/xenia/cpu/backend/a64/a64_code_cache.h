...
struct InstFix{
    static constexpr uint64_t TY_MOVK_X16_IMM16_SHIFT32=(1ull<<56);
    static constexpr uint64_t TY_MOVK_X17_IMM16_SHIFT32=(2ull<<56);
    static InstFix REPLACE_TO_MOVK_X16_IMM16_SHIFT32(uint32_t inst_offset){
        return InstFix(TY_MOVK_X16_IMM16_SHIFT32|inst_offset);
    }
    static InstFix REPLACE_TO_MOVK_X17_IMM16_SHIFT32(uint32_t inst_offset){
        return InstFix(TY_MOVK_X17_IMM16_SHIFT32|inst_offset);
    }

    uint64_t type() const{
        return value&0xFF00000000000000;
    }
    uint32_t inst_offset() const{
        return uint32_t(value&0xFFFFFFFF);
    }

    bool operator==(const InstFix& other) const {
    ...
    private:
    uint64_t value;
    InstFix(uint64_t v):value(v){}
};

struct InstFixHasher {
    size_t operator()(const InstFix& inst_fix) const {
        return std::hash<uint64_t>()(inst_fix());
    }
};
static_assert(sizeof(InstFix)==8);
...
class A64CodeCache : public CodeCache {
 ...
 protected:
  ...
  static const uintptr_t kIndirectionTableBaseLow = 0x80000000;
  ...
  static const uintptr_t kGeneratedCodeExecuteBaseLow = 0xA0000000;
